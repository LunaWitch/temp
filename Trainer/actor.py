...
class Actor:
    ...
    def _rollout(self, num_episode, num_trajectory):
        ...
        for i in range(num_trajectory):
            ...
            trajectory.append((state, next_state, value, next_value, action, reward, log_prob, ongoing, info))
            score += reward
            state = next_state
            value = next_value
            if ongoing:
    ...
    def get_episode(self, num_episode, num_trajectory):
        print(f'get_episode')
        trajectory, _ = self._rollout(num_episode, num_trajectory)

        states, next_states, values, next_values, actions, rewards, log_probs, ongoings, infos = zip(*trajectory)
        ...
        extra_data = self.model.preprocess_data(states, next_states, values, next_values, actions, rewards, log_probs, ongoings, infos)
    ...
    def get_score(self, num_episode, num_trajectory):
        _, score = self._rollout(num_episode, num_trajectory)
        return score
